    ...
    <div class="px-3 py-3 lg:px-5 lg:pl-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center justify-start">
          ...
          {{-- LOGO & NAMA APLIKASI DINAMIS DARI SETTINGS --}}
          <a href="{{ url('/') }}" class="flex ml-2 md:mr-24 items-center">
            @if(!empty($appSettings['app_logo']))
              <img src="{{ $appSettings['app_logo'] }}" class="h-8 mr-3 object-contain" alt="Logo" />
            @else
                ...
                <ul class="py-1" role="none">
                  ...
                  <li>
                    <form method="POST" action="{{ route('logout') }}">
                      @csrf
                      <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:text-red-400 dark:hover:bg-gray-600">
                        Logout
                      </button>
                    </form>
                  </li>
